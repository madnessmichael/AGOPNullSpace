...
os.environ["CUDA_VISIBLE_DEVICES"] = "2"  # Using GPU 1
...
def parse_args():
    ...
    parser.add_argument("--layers", type=str, required=False, default=None, help="Comma-separated list of layers used during embedding extraction (if --layers was passed to extract_embeddings.py)")
...
if __name__ == "__main__":
    ...
    if args.layers is not None:
        ...
        logger.info(f"Using --layers mapping: {layer_to_local}")
    else:
    ...
    for layer, ratio in layers_ratio_list:
        ...
        logger.info(f"layer: {layer} (local_idx: {local_idx}), ratio: {ratio}")

        # Calculate null space projection matrix for benign embeddings
        P_layer = null_space_projection_l(H_benign_train[:, local_idx, :], abs_nullspace_ratio=ratio)
        P[layer] = P_layer
        ...
        tilde_delta_layer = cal_tilde_delta_with_regularization_l(
            H_harmful_train[:, local_idx, :], P_layer, refusal_vectors[layer], lambda_reg=10.0, device=device)
